...
class CoordGate(nn.Module):
    def __init__(self, enc_channels, out_channels, enctype='pos', **kwargs):
    ...
    def forward(self, x):
        ...
        if self.enctype == 'pos':  # 位置编码处理
            ...
            x_coord = torch.linspace(-1, 1, h, device=x.device, dtype=x.dtype)
            y_coord = torch.linspace(-1, 1, w, device=x.device, dtype=x.dtype)
            pos = torch.stack(torch.meshgrid((x_coord, y_coord), indexing='ij'), dim=-1).view(-1, 2)
            # 编码
            # 使用编码器生成门控矩阵
            gate = self.encoder(pos).view(1, x.shape[2], x.shape[3], x.shape[1]).permute(0, 3, 1, 2)
...
if __name__ == '__main__':
    ...
    in_size=[256,256]
    encoding_layers = 2
    initialiser = torch.rand((32, 3))
    kwargs = {'encoding_layers': encoding_layers, 'initialiser': initialiser}
    block = CoordGate(32, 32, enctype = 'pos', **kwargs)
